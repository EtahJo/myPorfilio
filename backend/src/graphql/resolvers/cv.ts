...
module.exports={
    Query:{
        ...
        getEducations:async(_,{userId})=>{
            try{
                const educations = await Education.findById(userId).sort({createdAt:-1})
                if(educations){
                    return educations
                }else{
        ...
        getExperiences:async(_,{userId})=>{
            try{
                const experiences = await Experience.findById(userId).sort({createdAt:-1})
                if(experiences){
                    return experiences
                }else{
